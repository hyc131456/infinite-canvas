...
- [修复] 画布刷新或重试视频时优先按 ComfyUI 模型恢复本地任务，纠正误存的云端任务类型，避免错误要求 API Key。

+ [修复] 本地 ComfyUI 视频脚本模型允许 API Key 留空，不再被画布配置校验误拦截。
+ [调整] 画布视频任务轮询超时时间由约 5 分钟延长至约 12 分钟，ComfyUI 失败仍会立即结束。
+ [新增] 新增 Fisher 剧本资产视频本地 Skill，原样内置 MiniMax 文戏与武戏模板，并补充画布资产、分段、确认和生成流程。
+ [修复] 修复打开模型脚本编辑器时 ComfyUI Z-Image 模板错误解析外层插值，避免 `runId` 未定义导致页面崩溃。
+ [新增] 新增 ComfyUI 工作流图形参数编辑器 PRD 与可交互 HTML 原型，明确节点图、点击参数暴露和画布绑定方案。
+ [新增] 配置页 ComfyUI 工作流编辑器新增节点图、点击节点参数检查器、参数暴露编辑和缩放平移交互。
+ [调整] ComfyUI 工作流导入会根据 `VHS_VideoCombine` 自动识别视频能力，支持将 Video Combine 配置为输出节点并在视频工作台执行。
+ [新增] 新增画布连线绑定 ComfyUI 工作流参数的 PRD，明确正负提示词、自定义标量参数、校验、持久化和旧行为兼容方案。
+ [新增] 画布连线可将文本节点显式绑定到自定义 ComfyUI 工作流参数，生成和重试时执行类型与必填校验并保存参数快照。
+ [修复] ComfyUI 自定义工作流的显式 width/height 和负面提示词参数不再被画布默认值覆盖。
+ [新增] 配置页新增本地 ComfyUI 服务与 API 工作流管理，支持自定义节点参数、可选 API Key、工作流测试，并在生图工作台和画布中调用本地工作流生成图片。
+ [新增] 图片模型调用脚本新增 ComfyUI Z-Image 工作流模板，支持设置正面提示词、负面提示词及 width/height 并通过本地 ComfyUI 生图。
+ [优化] 画布生成的视频结果自动继承来源配置节点的分组，便于将视频提示词与生成视频分区整理。
+ [新增] 视频模型调用脚本新增 ComfyUI MiniMax H3 工作流模板，可上传三张画布参考图并将提示词、时长和画幅传入本地工作流生成视频。
+ [调整] 画布上的 ComfyUI MiniMax H3 视频设置支持逐节点填写像素量，并通过调用脚本写入工作流的 ResolutionSelector 节点。
+ [调整] 内置本地 ComfyUI MiniMax H3 视频模型，没有有效视频模型时自动选中并通过自定义调用脚本生成。
+ [修复] 生成配置声明参考节点但提示词未嵌入图片引用时，自动带入声明的参考图，避免误走文生图接口。
+ [修复] 资产设定图改为纯白背景单资产隔离生成，断开资产说明上下文引用，避免角色、道具和场景互相串入。
+ [调整] Canvas Agent 批量生成任务限制为最多 3 个并发，超出任务按提交顺序排队且失败不自动重试。
