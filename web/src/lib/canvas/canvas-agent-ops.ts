...
export type CanvasAgentOp =
    ...
    | { type: "connect_nodes"; id?: string; fromNodeId: string; toNodeId: string; parameterKey?: string }
...
export function applyCanvasAgentOps(snapshot: CanvasAgentSnapshot, ops?: CanvasAgentOp[]) {
    ...
    (Array.isArray(ops) ? ops : []).forEach((op, index) => {
        ...
        if (op.type === "connect_nodes") {
            ...
            if (!hasNodes) return;
            const existing = connections.find((conn) => conn.fromNodeId === op.fromNodeId && conn.toNodeId === op.toNodeId);
            if (existing) {
                if (op.parameterKey !== undefined) connections = connections.map((conn) => (conn.id === existing.id ? { ...conn, parameterKey: op.parameterKey.trim() || undefined } : conn));
            } else {
                connections = [...connections, { id: op.id || nanoid(), fromNodeId: op.fromNodeId, toNodeId: op.toNodeId, ...(op.parameterKey?.trim() ? { parameterKey: op.parameterKey.trim() } : {}) }];
            }
        }
