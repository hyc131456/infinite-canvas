...
export type ConfigTabKey = "channels" | "local-proxy" | "preferences" | "comfyui" | "prompt-sources" | "webdav" | "local-storage";
...
function comfyUiChannel(): ModelChannel {
    return { id: COMFYUI_CHANNEL_ID, name: "ComfyUI 本地", baseUrl: "http://127.0.0.1:8188", apiKey: "", apiFormat: "openai", models: [{ name: COMFYUI_MINIMAX_H3_MODEL, capability: "video", script: getComfyUiMinimaxH3Script() }] };
}
...
export function modelCapabilityOf(config: AiConfig, value: string): ModelCapability | undefined {
    ...
    if (comfyMatch?.service.enabled) return comfyMatch.workflow.capability;
...
export function selectableModelsByCapability(config: AiConfig, capability?: ModelCapability) {
    ...
    const comfyModels = !capability || capability === "image" || capability === "video" ? comfyUiModelOptions(getComfyUiServices(), capability === "image" || capability === "video" ? capability : undefined) : [];
...
function isAiConfigReady(config: AiConfig, model: string) {
    ...
    if (comfyMatch) return Boolean(comfyMatch.service.enabled && comfyMatch.service.baseUrl.trim());
    ...
    persist(
        ...
        }),
        {
            ...
            merge: (persisted, current) => {
                ...
                const comfyVideo = findComfyUiWorkflow(getComfyUiServices(), videoModel);
                const hasVideoModel = channels.some((channel) => channel.models.some((model) => model.capability === "video" && encodeChannelModel(channel.id, model.name) === videoModel))
                    || Boolean(comfyVideo?.workflow.capability === "video" && comfyVideo.service.enabled);
                return {
                    ...current,
