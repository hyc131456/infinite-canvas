...
function VideoSettingsPortal({
    ...
    return createPortal(
        ...
        >
            <VideoSettingsPanel config={config} onConfigChange={(key, value) => onConfigChange(key, value)} theme={theme} showMegapixels={config.model === "comfyui-local::minimax-h3-local"} className="space-y-4" />
        </div>,
        document.body,
    );
}
