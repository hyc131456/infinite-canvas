...
import { detectComfyUiOutputs, isComfyUiVideoOutputNode, parseComfyUiWorkflow, validateComfyUiWorkflow } from "@/services/comfyui/workflow-parser";
...
import { ComfyUiWorkflowGraphEditor } from "@/components/layout/comfyui-workflow-graph-editor";
...
function ComfyUiWorkflowModal({ state, service, onClose }: { state: WorkflowEditorState; service: ComfyUiService | null; onClose: () => void }) {
    ...
    return (
        <Modal open title={t("config.comfyui.workflowEditor")} width={1240} onCancel={onClose} onOk={save} okText={t("common.save")} cancelText={t("common.cancel")} destroyOnHidden styles={{ body: { padding: 0 } }}>
            <Tabs activeKey={activeTab} onChange={setActiveTab} items={[
                { key: "graph", label: t("config.comfyui.graphTab"), children: <ComfyUiWorkflowGraphEditor draft={draft} patch={patch} /> },
